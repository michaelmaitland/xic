...
public class IRSimulator {
        ...
        if (libraryFunctions.contains(name)) {
            ...
            if (frame.rets.isEmpty()) {
                ...
                if (frame.rets.size() != numReturnVals) {
                    throw new InternalCompilerError("Function '" + name +
                            "' declared to return " + numReturnVals +
                            " values, but it returned " + frame.rets.size());
                }

                for (int i = 0; i < numReturnVals; i++) {
                    parent.put(Configuration.ABSTRACT_RET_PREFIX + i, frame.rets.get(i));
                }
            }

            if (numReturnVals > 0) {
                return parent.get(Configuration.ABSTRACT_RET_PREFIX + 0);
            }
        }
        return 0;
    }
    ...
    public class ExecutionFrame {
        ...
        /** return values from this frame. Only used if the IRReturn node(s) for this function have children. */
